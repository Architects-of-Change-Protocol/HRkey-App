...
## ✅ `users` / `references` conflict resolved via live schema verification
...
**Resolved 2026-07-13:** the project owner verified `public.users` and
`public.references` directly against `schema_dump_live.sql` (a production
schema dump) and confirmed they match the structures already captured in
`supabase/migrations/`. See `docs/HIBERNATION_AUDIT.md` Phase 2.5 ("Final
schema verification") and Phase 7 (verdict: `STATUS: HIBERNATION READY`).
`analytics_events` and `hrscore_snapshots` were not individually
re-verified against the dump, but are not restorability blockers — see
table below.

## Known duplicate-name situations (flagged in filenames with `_CONFLICT_`)

| Situation | Files involved | Notes |
|---|---|---|
| `users` / `references` table shape | `sql/001_identity_and_permissions.sql`, `sql/003/007/020` (extend the same tables), `sql/030_trust_data_model_v1.sql`, `sql/dev-seeds/seed_hrkey_referrals.sql` | Three independent `CREATE TABLE IF NOT EXISTS users` (and two of `references`) with different columns. **Resolved**: live schema dump confirms the structures already captured here are correct. |
| `analytics_events` | `sql/008_analytics_layer.sql` vs `database/migrations/create_payment_tables.sql` | Two unrelated table shapes, same name. Not individually re-verified against the live dump; not a restorability blocker per the Phase 7 verdict. |
| `hrscore_snapshots` | `backend/migrations/20250115_create_hrscore_snapshots_table.sql` vs `backend/migrations/20250902_create_hrscore_snapshots_table.sql` | The 2025-09 version has RLS + a CHECK constraint the 2025-01 version lacks. Not individually re-verified against the live dump; not a restorability blocker per the Phase 7 verdict. |
